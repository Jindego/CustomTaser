...
local function CleanUp( ragdoll, ent, weapon, rendermode, movetype, time )
    ...
    ent:SetNWBool( "gstore_tased", false )
...
local function Ragdoll( ent )
    ...
    ent:SetNWBool( "gstore_tased", true )
    ...
    ent:SetNWEntity( "gstore_ragdoll_entity", ragdoll )
...
function ENT:Touch( ent )
    ...
    if ( speed > 200 and ( ent:IsPlayer() or ent:IsNPC() ) ) then
        ...
        if ( self.TaserHitSound == nil ) then
            ...
            ent:EmitSound( Sound( "taser/taser_hit.wav" ) )
        end
        
        Ragdoll( ent )
        self:SetParent( ent:GetNWEntity( "gstore_ragdoll_entity" ) )
        self:SetMoveType( MOVETYPE_NOCLIP )

        self.Target = ent

        local ragdoll = ent:GetNWEntity( "gstore_ragdoll_entity" )
		if ( not IsValid( ragdoll ) ) then return end
